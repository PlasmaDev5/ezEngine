...
void ezTokenizer::GetAllLines(ezHybridArray<const ezToken*, 32>& out_tokens) const
{
  out_tokens.Clear();
  out_tokens.Reserve(m_Tokens.GetCount());

  for (const ezToken& curToken : m_Tokens)
  {
    if (curToken.m_iType != ezTokenType::Newline)
    {
      out_tokens.PushBack(&curToken);
    }
  }
}

ezResult ezTokenizer::GetNextLine(ezUInt32& inout_uiFirstToken, ezHybridArray<ezToken*, 32>& out_tokens)
{
  out_tokens.Clear();

  ezHybridArray<const ezToken*, 32> Tokens0;
  ezResult r = GetNextLine(inout_uiFirstToken, Tokens0);
...
ezResult ezTokenizer::GetNextLine(ezUInt32& inout_uiFirstToken, ezHybridArray<const ezToken*, 32>& out_tokens) const
{
  out_tokens.Clear();

  const ezUInt32 uiMaxTokens = m_Tokens.GetCount() - 1;

  while (inout_uiFirstToken < uiMaxTokens)
  {
    const ezToken& tCur = m_Tokens[inout_uiFirstToken];

    // found a backslash
    if (tCur.m_iType == ezTokenType::NonIdentifier && tCur.m_DataView == "\\")
    {
      const ezToken& tNext = m_Tokens[inout_uiFirstToken + 1];
      ...
      if (tNext.m_iType == ezTokenType::Newline)
      {
        ...
        if (inout_uiFirstToken > 0 && m_Tokens[inout_uiFirstToken - 1].m_iType == ezTokenType::Identifier && inout_uiFirstToken + 2 < uiMaxTokens && m_Tokens[inout_uiFirstToken + 2].m_iType == ezTokenType::Identifier)
        {
          ezStringBuilder s1 = m_Tokens[inout_uiFirstToken - 1].m_DataView;
          ezStringBuilder s2 = m_Tokens[inout_uiFirstToken + 2].m_DataView;
          ezLog::Warning("Line {0}: The \\ at the line end is in the middle of an identifier name ('{1}' and '{2}'). However, merging identifier "
            ...
            m_Tokens[inout_uiFirstToken].m_uiLine, s1, s2);
        }

        // ignore this
        inout_uiFirstToken += 2;
        continue;
      }
    }
    ...
    if (m_Tokens[inout_uiFirstToken].m_iType == ezTokenType::Newline)
    {
      ++inout_uiFirstToken;
      return EZ_SUCCESS;
    }
